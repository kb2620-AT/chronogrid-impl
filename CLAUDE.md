...
lake env lean Chronogrid/Inverse.lean           # 16 axiom lines; 33 total, no `sorryAx`
```
...
- **No floating-point for time values.** `bigint` internally, `string` at API and DB boundaries.
  ...
  from there into the integrator. `sp3.ts` extends the same discipline to the orbit-file chain:
  SP3 numeric fields are fixed-width decimals and therefore exactly rational (position `N/10³` m,
  velocity `N/10⁷` m/s), the order-9 Lagrange evaluation runs over integer numerators, and ω is
  rational so `ω×r` adds no fourth rounding event. Its divider hides only the fixture writer's
  ECI→ECEF rotation.
