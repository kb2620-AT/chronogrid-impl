...
const targetLevel = parseInt(args[args.indexOf('--level') + 1] ?? '3', 10) as 1 | 2 | 3;
...
if (!jsonMode) {
  ...
  console.log(`  TAI now:      ${C.dim}CG:TAI:${Date.now() * 1_000_000n.toString()}/v1${C.reset}\n`);
}
...
async function main(): Promise<void> {
  ...
  for (const { name, cases } of SUITES) {
    ...
    if (!jsonMode && !quiet) {
      for (const t of result.tests) {
        ...
        if (t.status === 'fail') {
          console.log(`     ${C.red}Expected:${C.reset} ${JSON.stringify(t.expected)}`);
          console.log(`     ${C.red}Actual:${C.reset}   ${JSON.stringify(t.actual)}`);
        }
        if (t.status === 'error') {
  ...
  if (outputFile) {
    ...
    if (!jsonMode) console.log(`  ${C.dim}Report gespeichert: ${outputFile}${C.reset}\n`);
  }

  // Exit-Code
  const exitCode = conformance_level === 'none' ? 2
