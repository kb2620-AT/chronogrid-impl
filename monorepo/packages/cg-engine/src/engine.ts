...
import type { CTDDLDomain, CGTA, CGInterval, MachineID, CGFI } from 'cg-types/domain.js';
 ...
 */
export function safeAdd(a: bigint, b: bigint, level: 1 | 2 | 3 = 3): bigint {
  const result = a + b;
  if (level < 3) {
    if (result > INT64_MAX) throw Errors.ExtentError.Int64Overflow(result);
    if (result < INT64_MIN) throw Errors.ExtentError.Int64Overflow(result);
  }
  return result;
}
...
export function bigIntToBytesBigEndian(value: bigint): Uint8Array {
...
export function verifySerialization(): boolean {
  const zero = bigIntToBytesBigEndian(0n);
  if (zero.length !== 1 || zero[0] !== 0x00) return false;
  const twoFiftySix = bigIntToBytesBigEndian(256n);
  if (twoFiftySix.length !== 2 || twoFiftySix[0] !== 0x01 || twoFiftySix[1] !== 0x00) return false;
  return true;
}
...
export function computeMachineID(taiNs: bigint): MachineID {
...
export function computeCGFI(
  ...
  let offset = 0;
  input.set(taiPrefix, offset); offset += taiPrefix.length;
  input[offset++] = taiSign;
  input.set(taiBytes, offset); offset += taiBytes.length;
  input.set(contentHash, offset); offset += contentHash.length;
  input.set(typeBytes, offset); offset += typeBytes.length;
...
export function checkCGUARange(value: bigint): void {
  if (value < 0n || value > CGUAS_MAX) {
    throw Errors.CGUASError.AddressOutOfRange(value);
  }
}

export function cguaSafeAdd(base: bigint, offset: bigint): bigint {
...
export function checkExtent(value: bigint, domain: CTDDLDomain): void {
  if (domain.extent.min !== null) {
    const min = BigInt(domain.extent.min);
    if (value < min) throw Errors.ExtentError.UnderLowerBound(value, min);
  }
  if (domain.extent.max !== null) {
    const max = BigInt(domain.extent.max);
    if (value > max) throw Errors.ExtentError.OverUpperBound(value, max);
  }
}
...
export const Allen = {
  ...
  before:    (a: CGInterval, b: CGInterval): boolean => a.end < b.start,
  /** A after B: A.start > B.end */
  after:     (a: CGInterval, b: CGInterval): boolean => a.start > b.end,
  /** A meets B: A.end === B.start */
  meets:     (a: CGInterval, b: CGInterval): boolean => a.end === b.start,
  /** A metBy B: A.start === B.end */
  metBy:     (a: CGInterval, b: CGInterval): boolean => a.start === b.end,
  /** A overlaps B */
  overlaps:  (a: CGInterval, b: CGInterval): boolean =>
    a.start < b.start && a.end > b.start && a.end < b.end,
  /** A overlappedBy B */
  overlappedBy: (a: CGInterval, b: CGInterval): boolean =>
    b.start < a.start && b.end > a.start && b.end < a.end,
  /** A starts B: gleicher Start, A endet früher */
  starts:    (a: CGInterval, b: CGInterval): boolean =>
    a.start === b.start && a.end < b.end,
  /** A startedBy B */
  startedBy: (a: CGInterval, b: CGInterval): boolean =>
    a.start === b.start && a.end > b.end,
  /** A during B: A vollständig in B */
  during:    (a: CGInterval, b: CGInterval): boolean =>
    a.start > b.start && a.end < b.end,
  /** A contains B */
  contains:  (a: CGInterval, b: CGInterval): boolean =>
    a.start < b.start && a.end > b.end,
  /** A finishes B: gleicher Endpunkt, A startet später */
  finishes:  (a: CGInterval, b: CGInterval): boolean =>
    a.end === b.end && a.start > b.start,
  /** A finishedBy B */
  finishedBy:(a: CGInterval, b: CGInterval): boolean =>
    a.end === b.end && a.start < b.start,
  /** A equals B: identische Grenzen */
  equals:    (a: CGInterval, b: CGInterval): boolean =>
    a.start === b.start && a.end === b.end,

  /** Gibt alle zutreffenden Relationen zurück (Diagnosefunktion) */
  all: (a: CGInterval, b: CGInterval): string[] => {
    const rels: string[] = [];
    for (const [name, fn] of Object.entries(Allen)) {
      if (name === 'all') continue;
      if ((fn as (a: CGInterval, b: CGInterval) => boolean)(a, b)) rels.push(name);
    }
    return rels;
  },
};

// ── TAI ↔ UTC-Konvertierung (piecewise-linear) ───────────────────────────────
// Normative Schaltsekunden-Tabelle (CG-STD-2100 Anhang A, 27 Einträge 1972–2017)

interface LeapSecondEntry {
  utcSeconds: bigint;  // UTC-Zeitpunkt des Sprungs
  taiOffset: number;   // TAI-UTC nach diesem Zeitpunkt
}
...
export function taiOffsetAtUtc(utcSeconds: bigint): number {
...
export function cgfiToHex(cgfi: CGFI): string {
  return Buffer.from(cgfi).toString('hex');
}

/** MachineID als Hex-String */
export function machineIdToHex(mid: MachineID): string {
  return Buffer.from(mid).toString('hex');
}

// ── Re-Exports aus mapping.ts (für Rückwärtskompatibilität mit engine.js-Imports) ─
// handlers.ts und t-eng.ts importieren utcNsToTaiNs direkt aus 'cg-engine/engine.js'
export { utcNsToTaiNs, taiNsToUtcNs, utcToTai, taiToUtc } from './mapping.js';
