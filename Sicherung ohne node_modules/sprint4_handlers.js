...
import { computeMachineID, computeCGFI, cgfiToHex, machineIdToHex, } from 'cg-engine/engine.js';
...
export const convertTimepoint = async (req, ctx) => {
    verifyJWT(req.headers.authorization);
    const body = req.body;
    if (!body?.cgta)
        throw Errors.SyntaxError.MissingField('cgta');
...
export const getFile = async (req, ctx) => {
    verifyJWT(req.headers.authorization);
...
export const deleteFile = async (req, ctx) => {
    const auth = verifyJWT(req.headers.authorization);
    requireRole(auth, 'maintainer');
    const body = req.body;
    const reason = body?.reason ?? 'dsgvo_art17';
    await ctx.manifests.softDelete(req.params.cgfi, taiCGTA(ctx.now()), reason);
    return ok({
...
// ── 19. GET /v1/health ────────────────────────────────────────────────────────
export const getHealth = async (req, ctx) => {
    return ok({
        status: 'ok',
