 ...
 */
import { mod } from '../../cg-engine/src/engine.ts';
...
export function encodeGregorian(c) {
...
export function decodeGregorian(seconds) {
    if (seconds < 0n)
        throw new Error('Negative Sekunden nicht unterstützt in Gregorian v1');
...
export function verifyGregorianRoundtrip(c) {
    const encoded = encodeGregorian(c);
    const decoded = decodeGregorian(encoded);
    return (decoded.year === c.year &&
