...
import type { ConformanceLevel } from '../../cg-types/src/domain.ts';
...
export async function runTest(tc: TestCase): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      error: err instanceof Error ? `${err.message} (${(err as unknown as Record<string,unknown>).code ?? ''})` : String(err),
      duration_ms: Date.now() - start, level: tc.level, suite: tc.suite,
    };
  }
}
