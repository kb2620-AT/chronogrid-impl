 ...
 */
import { safeAdd, mod, bigIntToBytesBigEndian, verifySerialization, computeMachineID, computeCGFI, Allen, utcSecondsToTaiNs, } from './engine.ts';
...
test('T-BIG-006: safeAdd(INT64_MAX, 1) mit Level 1 wirft CG-E-003', () => {
    ...
    let threw = false;
    try {
        safeAdd(INT64_MAX, 1n, 1);
    }
    catch (e) {
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-003.004');
        }
    }
...
test('T-ENG-UC1-001: TAI Waypoint-Crossing = UTC + 37s', () => {
    ...
    const expectedTaiNs = (utcSeconds + 37n) * 1000000000n;
...
test('T-ENG-UC3-001: meets() für 100ms-Zeitreihe', () => {
    const m1 = { start: 1744035862300000000n, end: 1744035862400000000n, domain: 'energy', version: 1 };
    const m2 = { start: 1744035862400000000n, end: 1744035862500000000n, domain: 'energy', version: 1 };
...
test('T-ENG-UC3-002: TAI monoton über Sommerzeitumstellung', () => {
    ...
    const t1 = 1743209999000000000n; // kurz vor Umstellung
    const t2 = 1743210000000000000n; // kurz nach Umstellung
...
test('T-ENG-UC4-001: Cosmic-Age überschreitet INT64_MAX', () => {
    const INT64_MAX = 9223372036854775807n;
    const cosmicAgeNs = 435116774400000000000000n;
...
test('T-ENG-UC4-002: BigInt Allen.before() auf kosmologischer Skala', () => {
    const obs1967 = { start: 434693261180800000000000n, end: 434693261187200000000n, domain: 'cosmic', version: 1 };
    const obs1968 = { start: 434724797180800000000000n, end: 434724797184400000000n, domain: 'cosmic', version: 1 };
...
test('T-CTDDL-002: fehlendes Pflichtfeld wirft CG-E-001.002', () => {
    ...
    try {
        ...
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-001.002');
        }
    }
    assert.ok(threw);
});
test('T-CTDDL-003: ungültiger Domain-Typ wirft CG-E-001.004', () => {
    ...
    try {
        ...
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-001.004');
        }
    }
    assert.ok(threw);
});
test('T-CTDDL-004: stability=low ohne scientific_dependency wirft CG-E-008.001', () => {
    ...
    try {
        ...
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-008.001');
        }
    }
    assert.ok(threw);
});
test('T-CTDDL-005: Registry verhindert Rollback (CG-E-007.004)', () => {
    ...
    let threw = false;
    try {
        registry.register(parseCTDDL({ ...atcDomain, version: 1 }));
    }
    catch (e) {
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-007.004');
        }
    }
    assert.ok(threw);
});
