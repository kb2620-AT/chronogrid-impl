...
import { checkCGUARange, cguaSafeAdd } from '../../cg-engine/src/engine.ts';
import { bigIntToBytesBigEndian } from '../../cg-engine/src/engine.ts';
...
function computeSegmentHash(ownerId, start, end, grantedAt) {
...
function checkSegmentSize(sizeNs, level) {
    if (level < 0 || level > MAX_LEVEL) {
        throw new Error(`Ungültige Ebene: ${level} (muss 0–${MAX_LEVEL} sein)`);
    }
    const limits = SEGMENT_LIMITS[level];
...
export class SegmentRegistry {
    ...
    getById(segmentId) {
        ...
        if (!seg)
            throw new Error(`Segment nicht gefunden: ${segmentId}`);
        return seg;
    }
...
export function cguaToString(value) {
    checkCGUARange(value);
    return `CG:CGUAS:${value}/v1`;
}
export function parseCGUA(raw) {
    const m = /^CG:CGUAS:(\d+)\/v1$/.exec(raw);
    if (!m)
        throw new Error(`Ungültiges CGUA-Format: ${raw}`);
...
export function allocateFileAddress(segment, localOffset) {
