...
import type { CGError } from '../../cg-types/src/errors.ts';
...
export function verifyJWT(token: string | undefined): AuthContext {
  ...
  const auth: AuthContext = { sub: payload.sub, role: payload.cg_role, issuedAt: payload.iat };
  if (payload.cg_org !== undefined) auth.org = payload.cg_org;
  return auth;
}
