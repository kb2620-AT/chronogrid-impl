...
import { dispatch, ROUTES } from './handlers.ts';
import { makeTestJWT } from './middleware.ts';
import { OPENAPI_SPEC } from './openapi.ts';
import { InMemoryTimepointRepository, InMemoryDomainRepository, InMemoryManifestRepository, InMemoryRelationRepository, } from 'cg-storage/repository.js';
...
function authHeader(role = 'contributor') {
...
run('T-API-031: POST /relations/compute before', async () => {
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.relation, 'before');
});
run('T-API-032: POST /relations/compute during', async () => {
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.relation, 'during');
});
...
run('T-API-042: GET /segments/:id → 200', async () => {
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.segment_id, 'at.get.test');
});
run('T-API-043: GET /segments/resolve/:cgua → 200', async () => {
    ...
    const startAddr = postRes.body.start_address;
    // Adresse = start + 5
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.segment_id, 'at.resolve.test');
    assert.equal(res.body.local_offset, '5');
});
...
run('T-API-051: POST /files → 201 mit CGFI', async () => {
    ...
    assert.equal(body.cgfi.length, 64, 'CGFI muss 64 Hex-Zeichen sein');
});
run('T-API-052: GET /files/:cgfi → 200', async () => {
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.cgfi, cgfi);
});
...
run('T-API-054: CGFI-Determinismus — gleicher Inhalt → gleicher CGFI', async () => {
    ...
    assert.equal(r1.body.cgfi, r2.body.cgfi, 'Gleicher Inhalt muss gleichen CGFI erzeugen (I-R3)');
});
// ── T-API-06: Domains ─────────────────────────────────────────────────────────
console.log('\n── T-API-06: Domains ──');
