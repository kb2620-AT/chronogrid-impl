...
import { SegmentRegistry, cguaToString, parseCGUA, allocateFileAddress, } from './cguas.js';
import { InMemoryTimepointRepository, InMemoryDomainRepository, InMemoryManifestRepository, InMemoryRelationRepository, makeRelationRow, } from './repository.js';
...
test('T-CGUAS-005: resolve() wirft CG-E-010.002 für nicht belegte Adresse', () => {
    ...
    try {
        ...
        if (e && typeof e === 'object' && 'code' in e) {
            assert.ok(e.code.startsWith('CG-E-010'), `Erwartet CG-E-010.*, got ${e.code}`);
        }
    }
    assert.ok(threw);
});
...
test('T-CGUAS-007: I-D1 — gleiche segment_id wirft Fehler', () => {
    ...
    let threw = false;
    try {
        reg.allocate({ ...params, segment_id: 'at.doppelt' });
    }
    catch {
        threw = true;
    }
...
test('T-STOR-003: absolute_value muss BigInt sein', async () => {
    ...
    const row = {
        ...
        domain_id: 'TAI/v1', absolute_value: 1.5, // Float — verboten
        granularity: 'nanosecond', created_at: 'CG:TAI:1/v1', created_by: 'test',
    };
    let threw = false;
    try {
...
test('T-STOR-004: Bereichsabfrage auf timepoints', async () => {
    const repo = new InMemoryTimepointRepository();
    const base = 1743585310000000000n;
    for (let i = 0; i < 5; i++) {
        const t = base + BigInt(i) * 100000000n;
    ...
    const results = await repo.findByDomainAndRange('TAI/v1', base, base + 200000000n);
...
test('T-STOR-005: DomainRepository Insert + FindById', async () => {
    ...
    const domain = {
        name: 'test/domain', version: 1, semantics: 'time',
...
test('T-STOR-006: ManifestRepository Insert + FindByCGFI', async () => {
    ...
    const cgfi = cgfiToHex(computeCGFI(1743585310000000000n, content, 'aviation/atc-event/v1'));
...
test('T-STOR-007: logisches Löschen (Tombstone) setzt deleted_at', async () => {
    ...
    const cgfi = cgfiToHex(computeCGFI(1743585310000000000n, content, 'legal/contract/v1'));
    await repo.insert({
...
test('T-STOR-008: doppelter Tombstone wirft CG-E-011.009', async () => {
    ...
    let threw = false;
    try {
        await repo.softDelete(cgfi, 'CG:TAI:3/v1', 'dsgvo_art17');
    }
    catch (e) {
        threw = true;
        if (e && typeof e === 'object' && 'code' in e) {
            assert.equal(e.code, 'CG-E-011.009');
        }
    }
    assert.ok(threw);
});
test('T-STOR-009: Versions-Chain funktioniert korrekt', async () => {
    ...
    const cgfi1 = cgfiToHex(computeCGFI(1000000000n, c1, 'legal/contract/v1'));
    const cgfi2 = cgfiToHex(computeCGFI(2000000000n, c2, 'legal/contract/v1'));
    await repo.insert({
