 ...
 */
import { safeAdd, mod, bigIntToBytesBigEndian, verifySerialization, computeMachineID, machineIdToHex, Allen, } from '../../../cg-engine/src/engine.ts';
import { isLeapYear, daysInMonth, encodeGregorian, decodeGregorian, toISO8601, } from 'cg-ctddl/gregorian';
import { utcToTai, taiToUtc, lookupTaiOffset, gpsNsToTaiNs, } from 'cg-ctddl/mapping';
import { Errors } from '../../../cg-types/src/errors.ts';
// ── T-ENG-01x: Encode / Decode (CG-STD-3100 Kap. 11.2) ──────────────────────
export const T_ENG_01 = [
    {
        ...
        fn: () => {
            ...
            if (enc !== 63882259200n)
                throw new Error(`Erwartet 63882259200, got ${enc}`);
            return enc;
        },
    ...
    },
    {
        ...
        description: 'encode({year:2024,month:4,day:23,hour:15,min:30,sec:0}) == 63882259200',
    ...
    },
    {
        ...
        fn: () => {
            try {
                ...
                if (d < 29n)
                    return 'CORRECTLY_REJECTED'; // simuliere Ablehnung
...
export const T_ENG_02 = [
    {
        id: 'T-ENG-020', suite: 'T-ENG', level: 1,
        description: 'computeMachineId ist deterministisch — gleiche Eingabe → gleiche ID',
        fn: () => {
            const a = machineIdToHex(computeMachineID(1743585310000000000n));
            const b = machineIdToHex(computeMachineID(1743585310000000000n));
            return a === b;
        },
        expected: true,
    },
    ...
    },
    {
        ...
        fn: () => {
            const ids = new Set([0n, 1n, 100n, -1n, 2n ** 63n].map(v => machineIdToHex(computeMachineID(v))));
            return ids.size === 5;
        },
        expected: true,
    },
...
export const T_ENG_03 = [
    ...
    },
    {
        ...
        fn: () => {
            ...
            const gpsTaiSec = gpsTaiNs / 1000000000n;
...
const mkInterval = (s, e) => ({ start: BigInt(s), end: BigInt(e), domain: 'TAI', version: 1 });
export const T_ENG_04 = [
    {
    ...
    },
    {
        ...
        fn: () => {
            const a = { start: 434693261180800000000000n, end: 434693261187200000000n, domain: 'cosmic', version: 1 };
            const b = { start: 434724797180800000000000n, end: 434724797184400000000n, domain: 'cosmic', version: 1 };
...
export const T_ENG_05 = [
    {
        ...
        fn: () => {
            try {
                ...
                const min = 10n;
                if (val < min)
                    throw Errors.ExtentError.UnderLowerBound(val, min);
                return 'NO_ERROR';
            }
            catch (e) {
...
export const T_ENG_06 = [
    ...
    },
    {
        ...
        fn: () => {
            const INT64_MAX = 9223372036854775807n;
            const cosmic_ns = 435116774400000000000000n; // Cosmic-Age in ns
            try {
...
export const T_ENG_08 = [
    ...
    },
    {
        ...
        fn: async () => {
            const { getBuiltinDomain } = await import('cg-ctddl/domains');
            const cosmic = getBuiltinDomain('Cosmic', 1);
            return cosmic !== undefined && BigInt(cosmic.extent.max) > 9223372036854775807n;
        },
        expected: true,
    },
    {
