...
import type { CGRequest, CGResponse } from './middleware.ts';
...
} from '../../cg-storage/src/repository.ts';
import { makeRelationRow, makeDomainId } from '../../cg-storage/src/repository.ts';
import type { SegmentRegistry } from '../../cg-cguas/src/cguas.ts';
...
export const convertTimepoint: Handler = async (req, _ctx) => {
  verifyJWT(req.headers.authorization as string);
...
export const postFile: Handler = async (req, ctx) => {
  ...
  if (body.size_bytes !== undefined) row.size_bytes = body.size_bytes;
  if (body.prev_version !== undefined) row.prev_version = body.prev_version;
  if (body.cgua !== undefined) row.cgua = body.cgua;
  if (body.valid_from !== undefined) row.valid_from = body.valid_from;
  if (body.valid_until !== undefined) row.valid_until = body.valid_until;
  if (body.retention !== undefined) row.retention = body.retention;
  if (body.review_after !== undefined) row.review_after = body.review_after;
...
export const getFile: Handler = async (_req, ctx) => {
  verifyJWT(_req.headers.authorization as string);
...
export const deleteFile: Handler = async (_req, ctx) => {
  const auth = verifyJWT(_req.headers.authorization as string);
  requireRole(auth, 'maintainer');

  const body = _req.body as { reason?: string } | undefined;
...
// ── 19. GET /v1/health ────────────────────────────────────────────────────────
export const getHealth: Handler = async (_req, ctx) => {
  return ok({
    status:   'ok',
