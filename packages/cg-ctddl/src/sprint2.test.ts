...
import {
  ...
  toISO8601, verifyGregorianRoundtrip, type GregorianComponents,
} from './gregorian.ts';
import {
  utcToTai, taiToUtc, utcNsToTaiNs, taiNsToUtcNs,
  gpsNsToTaiNs, taiNsToGpsNs, convert,
  lookupTaiOffset,
} from './mapping.ts';
import { BUILTIN_DOMAINS, getBuiltinDomain } from './domains.ts';
...
test('T-ENG-011: Unix-Epoche 1970-01-01T00:00:00Z = 0 Sekunden', () => {
  ...
  const unixEpoch: GregorianComponents = { year: 1970n, month: 1n, day: 1n, hour: 0n, minute: 0n, second: 0n };
...
test('T-ENG-012: Roundtrip 2026-04-02T09:14:33Z', () => {
  const c: GregorianComponents = { year: 2026n, month: 4n, day: 2n, hour: 9n, minute: 14n, second: 33n };
...
test('T-ENG-013: Roundtrip 2026-03-29T01:59:59Z (kurz vor Sommerzeit)', () => {
  const c: GregorianComponents = { year: 2026n, month: 3n, day: 29n, hour: 1n, minute: 59n, second: 59n };
...
test('T-ENG-018: Roundtrip historisch 1582-10-15T00:00:00Z (Gregorianische Reform)', () => {
  const c: GregorianComponents = { year: 1582n, month: 10n, day: 15n, hour: 0n, minute: 0n, second: 0n };
  assert.ok(verifyGregorianRoundtrip(c));
});

test('T-ENG-019: Roundtrip Cosmic-Skala Jahr 12000', () => {
  const c: GregorianComponents = { year: 12000n, month: 6n, day: 15n, hour: 12n, minute: 0n, second: 0n };
  assert.ok(verifyGregorianRoundtrip(c));
});
...
test('T-ENG-035: GPS → TAI konstant +19s', () => {
  const gpsNs = 1743585273_000_000_000n;
  const taiNs = gpsNsToTaiNs(gpsNs);
  assert.equal(taiNs - gpsNs, 19n * 1_000_000_000n);
});
...
test('T-ENG-037: UTC→TAI Sub-Sekunden (Nanosekunden-Granularität)', () => {
  ...
  const utcNs  = utcMs * 1_000_000n;
  const taiNs  = utcNsToTaiNs(utcNs);
  const expectedOffset = 37n * 1_000_000_000n;
...
test('T-ENG-038: TAI→UTC Sub-Sekunden korrekt invertiert', () => {
  const utcNs  = 1744035825300_000_000n;
  const taiNs  = utcNsToTaiNs(utcNs);
...
test('T-ENG-039: convert() UTC→GPS in zwei Schritten', () => {
  ...
  const utcNs = 1743585273_000_000_000n;
  const gpsNs = convert(utcNs, 'UTC', 'GPS');
  // GPS = UTC + 37 - 19 = UTC + 18s
  const expectedDiff = (37n - 19n) * 1_000_000_000n;
...
test('T-DOM-004: Cosmic hat scientific_dependency (stability=low)', () => {
  ...
  assert.equal((cosmic.scientific_dependency as Record<string, unknown>).model, 'Planck-2018');
});
...
test('T-DOM-006: I-D1 — Built-in Domains sind eingefroren', () => {
  const tai = getBuiltinDomain('TAI', 1)!;
  assert.throws(() => {
    (tai as Record<string, unknown>).version = 99;
...
test('T-ENG-UC1-011: UTC→TAI→CGTA vollständiger Pfad', () => {
  ...
  const taiNs  = taiSec * 1_000_000_000n;          // → Nanosekunden
  assert.equal(taiNs, 1743585310_000_000_000n);    // CG-APP-0600 Testvektor
});

test('T-ENG-UC3-010: Energiemesswert TAI-Nanosekunden Sub-Sekunden', () => {
  // 2026-04-07T14:23:45.300Z
  const utcMs  = 1744035825300n;
  const taiNs  = utcNsToTaiNs(utcMs * 1_000_000n);
  assert.equal(taiNs, 1744035862300_000_000n,
    `UC3 TAI-Nanosekunden muss 1744035862300000000 sein, got ${taiNs}`);
});
